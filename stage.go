...
func (s *stage) Start() {
	//make関数はマップ、スライス、チャンネル（map[type]type, []type, chan）の確保と初期化を行い型そのものを得る関数
	...
	for {
		//「select」文は、「case」に指定したチャネルのうち、どれかを受信するまで待機します。
		select {
...
func (s *stage) observeTimer(tch chan bool) {
	for {
		tch <- true
		//時間差を表現する型はtime.Durationで、単位はナノ秒
...
func (s *stage) addBlocks(points []point) {
	for _, point := range points {
		//s.blocksにpointを連結する.pointにrangeで指定したpointsが代入される
